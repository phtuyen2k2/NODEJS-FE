...
const HeaderComponent = ({isHiddenSearch = false, isHiddenCart = false}) => {
    ...
    const content = (
        <div>
            ...
            {user?.isAdmin && (
              <WrapperContentPopup onClick={() => navigate('/system/admin')}>Trang ADMIN</WrapperContentPopup>
            )}
    ...
    return (
        <div style={{width: '100%', background: 'rgb(244,167,185)', display: 'flex', justifyContent: 'center'}}>
            <WrapperHeader style={{ justifyContent: isHiddenSearch && isHiddenSearch ? 'space-between' : 'unset' }}>
                <Col span={5}>
                    <WrapperTextHeader onClick={() => navigate('/')}>BLONKUTE</WrapperTextHeader>
                </Col>
                {!isHiddenSearch && (
                <Col span={13}>
                <ButtonInputSearch
